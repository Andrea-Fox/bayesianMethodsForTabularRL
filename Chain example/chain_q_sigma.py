...
def q_sigma(env, num_episodes=500, learning_rate=0.5, discount_factor=1, exploration_rate = 0.1, n = 5, num_states = 50, sigma = -1, learning_policy = ""):
    ...
    ep_rewards = []
    Q = np.ones((num_states, 2)) * 0.5  
    possible_actions = 2
    probabilities = np.ones((num_states, possible_actions))*(1/possible_actions)
        ...
        while tau != terminal_time -1 :   
            ...
            if (t < terminal_time):
                ...
                if done:
                    ...
                    if learning_policy == "greedy": 
                        next_action, state_counts, probability_vector = egreedy_policy(Q, next_state, state_counts, 0.0, possible_actions = 2)
                    elif learning_policy == "constant-epsilon":
                        epsilon = exploration_rate
                        next_action, state_counts, probability_vector = egreedy_policy(Q, next_state, state_counts, exploration_rate, possible_actions = 2)
                    elif learning_policy == "epsilon_greedy": 
                        epsilon = exploration_rate/(state_counts[state]+1)
                        next_action, state_counts, probability_vector = egreedy_policy(Q, next_state, state_counts, exploration_rate, possible_actions = 2)
                    elif learning_policy == "boltzmann_exploration":
                        next_action, state_counts, probability_vector = boltzmann_exploration(Q, next_state, state_counts, exploration_rate, possible_actions = 2)
                    else:
...
def wrapper_parallel_computing(n_episodes= 500, learning_rate = 0.1, discount_factor = 1, n = 1, n_simulations = 50, num_states = 5, sigma = 0, exploration_rate = 0.1, learning_policy = ""):
    ...
    for simulation_index in range(n_simulations):
        ...
        ep_rewards, Q = q_sigma(env, num_episodes = n_episodes, learning_rate = learning_rate,  discount_factor = discount_factor, exploration_rate= exploration_rate, n = n, num_states = num_states, sigma = sigma, learning_policy= learning_policy)
    ...
    for episode_index in range(n_episodes):
        ...
        for simulation_index in range(n_simulations):
            average_value[simulation_index] = ep_rewards_list[simulation_index][episode_index]
    ...
    for index_episode in range(n_episodes):
        ...
        for index_simulation in range(n_simulations):
            values[index_simulation] = ep_rewards_list[index_simulation][index_episode]
    ...
    for episode_index in range(n_episodes- episodes_considered, n_episodes):
        average_value = np.zeros((n_simulations, 1))
        for simulation_index in range(n_simulations):
            values_to_consider[simulation_index, episode_index - n_episodes + episodes_considered] = ep_rewards_list[simulation_index][episode_index]
    ...
    # return mean_variance_dataframe
...
n_values = [1, 5, 10, 20]   # [1, 2] + [i for i in range(5, 51, 5)] + [i for i in range(75, 251, 25)] # 1, 2, 5, 10, ..., 45, 50, 60, 70, ..., 240, 250
...
exploration_rates = [0, 0.1, 0.1, 1.5]
...
lista_possibili_gruppi_di_parametri = [ (1, n_episodes, "boltzmann_exploration", 1.5, 10)]
...
pool.close()
dataframe_results = pd.DataFrame(np.zeros((n_episodes, len(lista_possibili_gruppi_di_parametri))))
for i in range(len(lista_possibili_gruppi_di_parametri)):
    dataframe_results.iloc[:, i] = results[i]

dataframe_results.columns = lista_possibili_gruppi_di_parametri
# dataframe_results.to_csv("/home/andrea/Desktop/tesiReinforcmentLearning/codice/chain/risultati_numerici/chain_q_sigma_optimal_values.csv", index = False)

# dataframe_results.to_csv("/home/andrea/Desktop/tesiReinforcmentLearning/codice/chain/risultati_numerici/risultato_q_sigma_best_values.csv", index = False)
print(dataframe_results)
# dataframe_values = pd.DataFrame(np.zeros((len(lista_possibili_gruppi_di_parametri), 2)))
# 
# for i in range(len(lista_possibili_gruppi_di_parametri)):
...
# 
# 
# dataframe_values.index = lista_possibili_gruppi_di_parametri
# dataframe_values.to_csv("/home/andrea/Desktop/tesiReinforcmentLearning/codice/chain/risultati_numerici/risultato_q_sigma.csv", index = True)

for i in range(len(lista_possibili_gruppi_di_parametri)):
        plt.plot(pd.Series(dataframe_results.iloc[:, i]).rolling(50, min_periods = 10).mean() ) #, color = colors[index_color]   )
