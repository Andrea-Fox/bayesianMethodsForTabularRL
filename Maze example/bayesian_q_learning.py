...
def myopic_vpi_selection(q_values = 0, state = 0, possible_actions = 2, first_step = False):
    if np.max(q_values[state, :, 0]) == np.min(q_values[state, :, 0]) and np.max(q_values[state, :, 1]) == np.min(q_values[state, :, 1]) and np.max(q_values[state, :, 2]) == np.min(q_values[state, :, 2]) and np.max(q_values[state, :, 3]) == np.min(q_values[state, :, 3]):        # print("parametri uguali")
        return np.random.choice(possible_actions)
    else:
...
def bayesian_n_step_q_learning(env, num_episodes=500,  discount_factor=0.99, action_selection = "", values_update = "", n_steps = -1):  # buoni risultati: [0, .1, 1.001, 1], [0, .1, 1.01, .05] , [0, 5, 1.01, 0.01]
    ...
    if action_selection == "greedy_selection":
        Q = np.ones((env.num_states , 4, 4)) * [0, .0001, 4, .5]  
        n_steps = 1  
    elif action_selection == "q_value_sampling":
        Q = np.ones((env.num_states , 4, 4)) * [0, .25, 1.5, .01]
        n_steps = 1
    elif action_selection == "myopic_vpi_selection":
        Q = np.ones((env.num_states , 4, 4)) * [0, .0001, 4, .75]
        n_steps = 1
    ep_rewards = []
    episodes_length = []
    ...
    for episode_index in range(num_episodes):
        ...
        while tau < terminal_time -1:     
            ...
            if (t < terminal_time):
                ...
                if action_selection == "greedy_selection":
                    action = greedy_selection(Q, state, t, possible_actions=4) 
                elif action_selection == "q_value_sampling":
                    action = q_value_sampling_selection(Q, state, possible_actions = 4)
                elif action_selection == "myopic_vpi_selection":
                    action = myopic_vpi_selection(Q, state, possible_actions = 4)
                future_actions.append(action)
...
def wrapper_parallel_computing(action_selection = "", values_update = "", simulation_index = 0, n_episodes = 1000, n_steps = -1):
    ...
    env = Maze()
    rewards_sum, Q = bayesian_n_step_q_learning(env, num_episodes= n_episodes, discount_factor= 0.99, action_selection = action_selection, values_update = values_update, n_steps = n_steps)
...
mean_variance_dataframe = pd.DataFrame(np.zeros((len(lista_possibili_metodi), 2)))  #CAMBIARE!!!
...
mean_variance_dataframe.to_csv("data/mean_variance_rewards_bayesian_q_learning_final.csv", index = False)
results_dataframe.to_csv("data/comparison_rewards_bayesian_q_learning_final.csv", index = False)
