...
def bayesian_n_step_expected_sarsa(env, num_episodes=500,  discount_factor=0.99, action_selection = "", values_update = "", n_steps = -1):  # buoni risultati: [0, .1, 1.001, 1] , [0, .1, 1.1, .25]
    ...
    if action_selection  == "greedy_selection":
        Q = np.ones((env.possible_states , possible_actions, 4)) * [0, 0.01, 2, 0.25]    
    elif action_selection == "q_value_sampling":
        Q = np.ones((env.possible_states , possible_actions, 4)) * [0, 0.1, 1.5, 0.1]    
    elif action_selection == "myopic_vpi_selection":
        Q = np.ones((env.possible_states , possible_actions, 4)) * [0, .01, 1.5, 0.25]     # for each (state, action) pair we store the parameters of the corresponding distribution
    n_steps = 1
    ep_rewards = []
    state_counts = np.zeros((env.possible_states, ))
    action_probability = np.ones((env.possible_states, possible_actions)) * 0.5
    ...
    for episode_index in range(num_episodes):
        ...
        while  done == False:   # tau != terminal_time - 1 :     
            ...
            if action_selection  == "greedy_selection":
                action, action_probabilities, state_counts = greedy_selection(Q, state_index, state_counts, exploration_rate = 1, possible_actions= possible_actions) 
            elif action_selection == "q_value_sampling":
                action, action_probabilities = q_value_sampling_selection(Q, state_index, possible_actions = possible_actions)
            elif action_selection == "myopic_vpi_selection":
                action, action_probabilities, state_counts = myopic_vpi_selection(Q, state_index, possible_actions = possible_actions, state_counts = state_counts)
            action_probability[state_index, :] = action_probabilities
            future_actions.append(action)
            ...
            future_states_index.append(next_state_index)        # future_states[t+1]
            reward_sum += reward
            ...
            expected_value = dot( action_probability[future_states_index[t + 1]]  , Q[future_states_index[t + 1], :, 0]  )
            M1 = dot(relevant_rewards, gamma_list) + discount_factor **  n_steps * expected_value
            square = [ (Q[future_states_index[t + 1], i, 1] +1 )/(Q[future_states_index[t + 1], i, 1]) * (Q[future_states_index[t + 1], i, 3])/(Q[future_states_index[t + 1], i, 2] -1) + Q[future_states_index[t + 1], i, 0]**2   for i in range(possible_actions) ] 
            M2 = (dot(relevant_rewards, gamma_list))**2 + 2 * dot(relevant_rewards, gamma_list) * expected_value + discount_factor**2 * dot( square_vector(action_probability[future_states_index[t + 1]]) , square ) + 2 * discount_factor**2 * doppio_prodotto(action_probability, Q, possible_actions = possible_actions, next_state= future_states_index[t+1])
            
            mu_0 = Q[future_states_index[t], future_actions[t], 0]
            lambda_0 = Q[future_states_index[t], future_actions[t], 1]
            alpha_0 = Q[future_states_index[t], future_actions[t], 2]
            beta_0 = Q[future_states_index[t], future_actions[t], 3]
            ...
            Q[future_states_index[t], future_actions[t], 0] = mu_1
            Q[future_states_index[t], future_actions[t], 1] = lambda_1
            Q[future_states_index[t], future_actions[t], 2] = alpha_1
            Q[future_states_index[t], future_actions[t], 3] = beta_1
            t += 1
