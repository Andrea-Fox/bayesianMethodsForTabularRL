...
def greedy_selection(q_values, state, possible_actions = 2):
    ...
    if np.max(q_values[state, :, 0]) == np.min(q_values[state, :, 0]) and np.max(q_values[state, :, 1]) == np.min(q_values[state, :, 1]) and np.max(q_values[state, :, 2]) == np.min(q_values[state, :, 2]) and np.max(q_values[state, :, 3]) == np.min(q_values[state, :, 3]):
        ...
        return argmaxrand(q_values[state,: , 0])


def q_value_sampling_selection(q_values, state_index, possible_actions):

    if np.max(q_values[state_index, :, 0]) == np.min(q_values[state_index, :, 0]) and np.max(q_values[state_index, :, 1]) == np.min(q_values[state_index, :, 1]) and np.max(q_values[state_index, :, 2]) == np.min(q_values[state_index, :, 2]) and np.max(q_values[state_index, :, 3]) == np.min(q_values[state_index, :, 3]):
...
def myopic_vpi_selection(q_values = 0, state = 0, possible_actions = 2):
    if np.max(q_values[state, :, 0]) == np.min(q_values[state, :, 0]) and np.max(q_values[state, :, 1]) == np.min(q_values[state, :, 1]) and np.max(q_values[state, :, 2]) == np.min(q_values[state, :, 2]) and np.max(q_values[state, :, 3]) == np.min(q_values[state, :, 3]):
        # print("parametri uguali")
        return np.random.choice(possible_actions)
    else:
...
def bayesian_n_step_q_learning(env, num_episodes=500,  discount_factor=0.99, action_selection = "", values_update = "", n_steps = -1):  # buoni risultati: [0, .1, 1.001, 1], [0, .1, 1.01, .05] , [0, 5, 1.01, 0.01]

    if action_selection == "greedy_selection":
        Q = np.ones((env.possible_states , 3, 4)) * [0, .25, 1.01, 2]  
        n_steps = 1  
    elif action_selection == "q_value_sampling":
        Q = np.ones((env.possible_states , 3, 4)) * [0, .25, 2, 0.05]
        n_steps = 1
    elif action_selection == "myopic_vpi_selection":
        Q = np.ones((env.possible_states , 3, 4)) * [0, .0001, 2, 1]
        n_steps = 1
    ep_rewards = []
    episodes_length = []

    state_count = np.zeros((env.possible_states, ))
    ...
    for episode_index in range(num_episodes):
        ...
        future_states_index.append(state_index)   
        ...
        while tau < terminal_time -1:     
            ...
            if (t < terminal_time):
                ...
                state_index = future_states_index[t]
                state_count[state_index] += 1

                if action_selection == "greedy_selection":
                    action = greedy_selection(Q, state_index, possible_actions=3) 
                elif action_selection == "q_value_sampling":
                    action = q_value_sampling_selection(Q, state_index, possible_actions = 3)
                elif action_selection == "myopic_vpi_selection":
                    action = myopic_vpi_selection(Q, state_index, possible_actions = 3)
                future_actions.append(action)
                ...
                future_states_index.append(next_state_index)

                reward_sum += reward
                future_rewards.append(reward)    
                if t >= env._max_episodes_steps:
                    done = True
            ...
            if tau >= 0:
                ...
                if min(tau + n_steps , terminal_time) == tau+n_steps :
                    best_action = np.argmax(Q[future_states_index[tau + n_steps], :, 0])
                    M1 = dot(relevant_rewards, gamma_list) + discount_factor **  n_steps * Q[future_states_index[tau + n_steps], best_action, 0]
                    M2 = (dot(relevant_rewards, gamma_list))**2 + 2 * discount_factor ** n_steps * dot(relevant_rewards, gamma_list) * Q[future_states_index[tau + n_steps], best_action, 0] + (discount_factor ** (2*n_steps)) * ( ( (Q[future_states_index[tau + n_steps], best_action, 1] + 1)/(Q[future_states_index[tau + n_steps], best_action, 1]) )*(Q[future_states_index[tau + n_steps], best_action, 3]/(Q[future_states_index[tau + n_steps], best_action, 2]-1)) + (Q[future_states_index[tau + n_steps], best_action, 0])**2   )
                ...
                mu_0 = Q[future_states_index[tau], future_actions[tau], 0]
                lambda_0 = Q[future_states_index[tau], future_actions[tau], 1]
                alpha_0 = Q[future_states_index[tau], future_actions[tau], 2]
                beta_0 = Q[future_states_index[tau], future_actions[tau], 3]
                ...
                Q[future_states_index[tau], future_actions[tau], 0] = mu_1
                Q[future_states_index[tau], future_actions[tau], 1] = lambda_1
                Q[future_states_index[tau], future_actions[tau], 2] = alpha_1
                Q[future_states_index[tau], future_actions[tau], 3] = beta_1
              
            t += 1
           
        ep_rewards.append(reward_sum)
        episodes_length.append(terminal_time)
...
def wrapper_parallel_computing(action_selection = "", values_update = "", simulation_index = 0, n_episodes = 1000, n_steps = -1):
    ...
    rewards_sum, Q, episode_length = bayesian_n_step_q_learning(env, num_episodes= n_episodes, discount_factor= 0.99, action_selection = action_selection, values_update = values_update, n_steps = n_steps)
...
mean_methods = np.zeros((len(lista_possibili_metodi)))
...
for learning_policy_index in range( len(lista_possibili_metodi) ):
    ...
    print(sums)
    mean_methods[learning_policy_index] = np.mean(sums)
    print(np.mean(sums))
    
    sd_methods[learning_policy_index] = math.sqrt(np.var(sums))
    print(math.sqrt(np.var(sums)))
    
mean_variance_dataframe.iloc[:, 0] = mean_methods
...
mean_variance_dataframe.to_csv("data/mean_variance_rewards_bayesian_q_learning.csv", index = False)
results_dataframe.to_csv("data/comparison_rewards_bayesian_q_learning.csv", index = False)
