...
def q_sigma(env, num_episodes=500, learning_rate=0.5, discount_factor=1, exploration_rate = 0.1, n = 5, num_states = 50, sigma = -1, learning_policy = ""):
    ...
    terminal_times = []
    possible_actions = 3 
    Q = np.zeros((env.possible_states, possible_actions))  
    
    probabilities = np.ones((env.possible_states, possible_actions))*(1/possible_actions)

    state_counts = np.zeros((env.possible_states, 1))
    ...
    for index_episode in range(num_episodes):
        ...
        if learning_policy == "greedy": 
            action, state_counts, probability_vector = egreedy_policy(Q, state_index, state_counts, 0.0, possible_actions = possible_actions)
        elif learning_policy == "constant-epsilon":
            action, state_counts, probability_vector = egreedy_policy(Q, state_index, state_counts, exploration_rate, possible_actions = possible_actions)
        elif learning_policy == "epsilon_greedy": 
            epsilon = exploration_rate / (state_counts[state_index]+1)
            action, state_counts, probability_vector = egreedy_policy(Q, state_index, state_counts, epsilon, possible_actions = possible_actions)
        elif learning_policy == "boltzmann_exploration":
            action, state_counts, probability_vector = boltzmann_exploration(Q, state_index, state_counts, exploration_rate, possible_actions = possible_actions)
        probabilities[state_index, :] = probability_vector
        ...
        while tau != terminal_time -1 :   
            ...
            if (t < terminal_time):
                ...
                if done:
                    ...
                    delta = reward - Q[state_index, action]
                    future_delta.append(delta)

                else:
                    if learning_policy == "greedy": 
                        next_action, state_counts, probability_vector = egreedy_policy(Q, next_state_index, state_counts, 0.0, possible_actions = possible_actions)
                    elif learning_policy == "constant-epsilon":
                        epsilon = exploration_rate
                        next_action, state_counts, probability_vector = egreedy_policy(Q, next_state_index, state_counts, exploration_rate, possible_actions = possible_actions)
                    elif learning_policy == "epsilon_greedy": 
                        epsilon = exploration_rate/(state_counts[state_index]+1)
                        next_action, state_counts, probability_vector = egreedy_policy(Q, next_state_index, state_counts, exploration_rate, possible_actions = possible_actions)
                    elif learning_policy == "boltzmann_exploration":
                        next_action, state_counts, probability_vector = boltzmann_exploration(Q, next_state_index, state_counts, exploration_rate, possible_actions = possible_actions)
                    else:
                        print(learning_policy)

                    probabilities[next_state_index, :] = probability_vector
                    future_actions.append(next_action)

                    expected_value = dot(probability_vector, Q[next_state_index, :])
                    
                    
                    # delta = reward  + value_function[next_state] - value_function[state]
                    delta = reward + discount_factor * ( compute_sigma(sigma, index_episode+1) * Q[next_state_index, next_action] + (1-compute_sigma(sigma, index_episode+1)) * expected_value ) - Q[state_index, action]
                    future_delta.append(delta)
            ...
            if tau >= 0:
                ...
                return_G = Q[ future_states_index[tau] , future_actions[tau]]
                ...
                for k in range(tau, final_index+1):
                    ...
                    E *= discount_factor * ((1 - compute_sigma(sigma, index_episode)) * probabilities[future_states_index[k], future_actions[k]] + compute_sigma(sigma, index_episode) )    
                ...
                Q[ future_states_index[tau], future_actions[tau] ] += learning_rate * (return_G  - Q[ future_states_index[tau], future_actions[tau]] )
...
lista_possibili_gruppi_di_parametri = [(.25, n_episodes, "boltzmann_exploration", .1, 1), (.5, n_episodes, "greedy", 0, 5), (0.5, n_episodes, "epsilon_greedy", .1, 50), (0.5, n_episodes, "constant-epsilon", 0.1, 50), (0, n_episodes, "boltzmann_exploration", 1.5, 5) ]
...
dataframe_values.to_csv("data/mountain_car_q_sigma_optimal_values.csv", index = False)
